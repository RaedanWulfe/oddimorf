...
# share_engine_name = 'PROCESSOR_ENGINE'
            ...
            while self.is_running:
                ...
                for packet in self.channel.unpack():
                    ...
                    struct.pack_into(self.channel.struct_format, buffer, header_size_bytes + rabbit,
                                     int(packet[0]), float(packet[1]), float(packet[2]), float(packet[3]), float(packet[4]))
                    rabbit = (
...
class Interpreter(threading.Thread):
    ...
    def update_control_data(self):
        with open(os.path.join(os.path.dirname(__file__), f"_Control.dat"), 'r+b') as file:
            ...
            # -----------------------------------------------------------------
            # Custom fields
            struct.pack_into('<I', buffer, 16, self._plot_threshold)

    # Control data transfer functions
    def set_plot_threshold(self, value):
        if self._plot_threshold != value:
            self._plot_threshold = value
...
async def loop_async(context, _):
    ...
    # -------------------------------------------------------------------------
    # Incoming/Read queue
    readers.append(Reader(context.input_channel, 'Raw', context.sensor_origin))
    # -------------------------------------------------------------------------
    ...
    threshold_slider.set_map_range(2*8)
    threshold_slider.observe(
        'received', lambda: interpreter.set_plot_threshold(threshold_slider.value))
    interpreter.set_plot_threshold(threshold_slider.value)
    # -------------------------------------------------------------------------
